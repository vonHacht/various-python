...
class RBTree():
        ...
        while k.parent.color == 1:  # While parent is red
            if k.parent == k.parent.parent.right:  # if parent is right child of its parent
                ...
                if u.color == 1:  # if color of right child of grandparent i.e, uncle node is red
                    ...
                    k.parent.parent.color = 1
                    self.RR(
                        k.parent.parent)  # Call right rotate on grandparent
    ...
    def delete_node_helper(self, node, key):
        ...
        if z.left == self.NULL:  # If left child of z is NULL
            ...
            self.__rb_transplant(
                z, z.right)  # Transplant Node to be deleted with x
        elif z.right == self.NULL:  # If right child of z is NULL
            x = z.left  # Assign left child of z to x
            self.__rb_transplant(
                z, z.left)  # Transplant Node to be deleted with x
    ...
    def __traverse_simple(self, node):
        if node != self.NULL:
            self.__traverse_simple(node.left)
            print(f"\n{node.val}")
            self.__traverse_simple(node.right)

    def traverse_simple(self):
        self.__traverse_simple(self.root)
